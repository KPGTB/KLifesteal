...
public class CompressedSoulItem extends KItem {
    private final LifestealWrapper wrapper;

    public CompressedSoulItem(ToolsObjectWrapper wrapper, String fullItemTag) {
    ...
    public ItemStack getItem() {
        return new ItemBuilder(Material.IRON_NUGGET)
                .displayname(wrapper.getLanguageManager().getSingleString(LanguageLevel.PLUGIN, "compressedSoulName"))
                .lore(wrapper.getLanguageManager().getString(LanguageLevel.PLUGIN, "compressedSoulLore"))
                .model(2001)
                .glow()
                .build();
    }
