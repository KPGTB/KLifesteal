...
import pl.kpgtb.klifesteal.item.HeartItem;
import pl.kpgtb.klifesteal.item.SoulItem;
import pl.kpgtb.klifesteal.util.LifestealWrapper;

public class HeartRecipe extends KRecipe {
    private final NamespacedKey key;
    private final LifestealWrapper wrapper;

    public HeartRecipe(NamespacedKey recipeKey, ToolsObjectWrapper toolsObjectWrapper) {
    ...
    public Recipe getRecipe() {
        ShapedRecipe recipe = new ShapedRecipe(key,wrapper.getItemManager().getCustomItem(wrapper.getTag(), HeartItem.class));
        recipe.shape("dad", "asa", "dad");

        ItemStack soul = wrapper.getItemManager().getCustomItem(wrapper.getTag(), CompressedSoulItem.class);
